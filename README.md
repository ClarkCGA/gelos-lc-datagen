...
- v0.40: This version provides dem, lc2l2 8-9, Sentinel 2, and Sentinel 1 data for each chip. File names follow this convention for dem: ```dem_{chip_id:06}.tif``` and this convension for mutlitemporal platforms (s1rtc, s2l2a, lc2l2): ```{platform}_{chip_id:06}_{date}.tif```. PNG visualization are included for multitemporal data sources wth the same naming convention, substituting ```png``` for ```tif```. The dataset consists of 77,547 chips. A chip tracker metadata GeoJSON named ```cleaned_df.geojson``` is included with the following fields:
  ...
  - s2l2a_dates (str(list(int))): A list of 4 integer dates in format YYYYMMDD (no dashes) representing the days of each Sentinel 2 observation stored as a string. This can be read as a list using string literal eval.
  - s1rtc_dates (str(list(int))): A list of 4 integer dates in format YYYYMMDD (no dashes) representing the days of each Sentinel 1 observation stored as a string.
  - lc2l2_dates (str(list(int))):: A list of 4 integer dates in format YYYYMMDD (no dashes) representing the days of each lc2l2 observation stored as a string.
  - lulc (int): The land cover code of the chip, taken from the [10m annual lulc dataset](https://collections.sentinel-hub.com/impact-observatory-lulc-map/readme.html)
