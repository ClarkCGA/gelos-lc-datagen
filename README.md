...
- v0.50.1: This version adds new AOIs for better geospatial representation. File names follow this convention for dem: ```dem_{chip_id:06}.tif``` and this convention for multitemporal platforms (s1rtc, s2l2a, lc2l2): ```{platform}_{chip_id:06}_{date}.tif```. PNG visualizations are included for multitemporal data sources wth the same naming convention, substituting ```png``` for ```tif```. The dataset consists of 78,585 chips. A chip tracker metadata GeoJSON named ```gelos_chip_tracker.geojson``` is included with the following fields:
